...
//https://godoc.org/github.com/armon/go-proxyproto
var (
...
// 侦听器用于包装基础侦听器，该侦听器的连接可能使用HAProxy代理协议（版本1）。如果连接使用协议，则RemoteAddr（）将返回正确的客户端地址。
// （可选）定义ProxyHeaderTimeout以设置接收代理协议标头的最长时间。零表示没有超时。
...
// Conn用于包装可能称为“代理协议”的基础连接。如果它是，RemoteAddr（）将返回客户端的地址，而不是代理地址。
type Conn struct {
...
// NewConn用于包装可能说代理协议的net.Conn到proxyproto.Conn
...
// 进行初始扫描时，读取是检查代理协议标头。如果解析标头时发生错误，则将其返回并关闭套接字。
...
// before Read()
// 如果正在使用代理协议，RemoteAddr将返回客户端的地址，否则仅返回套接字对等方的地址。如果解析标头时发生错误，则不返回客户端的地址，并且关闭套接字。
// 这就暗示了如果客户端运行缓慢，则呼叫可能会阻塞。如果在Read（）之前调用了最后期限，则建议使用
